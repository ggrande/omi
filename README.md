...
This fork is currently focused on **Omi Ambient Companion**: a standalone Android companion app for local-first ambient capture that syncs to the official Omi API after user sign-in.

The original Omi source is still present in this repository for compatibility and reference while the companion stabilizes. The companion app is separate from the published Omi app and installs as:
...
- `.github/workflows/ambient-companion-android.yml` - debug APK build workflow for the companion.
...
This repository began as a fork of `BasedHardware/omi`. Upstream Omi source remains in the tree during companion development.
